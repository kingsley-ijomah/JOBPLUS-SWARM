...
echo "*/3 * * * * PGPASSWORD=repl_password pg_basebackup -h proxy -p 5432 -U repluser -D \"/backups/$(date +%Y%m%d-%H%M%S)\" -Fp -X stream > /var/log/pg_basebackup.log 2>&1" > /etc/cron.d/pg_basebackup
